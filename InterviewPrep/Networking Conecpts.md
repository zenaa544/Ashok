...
SDN: What and Why
•	Definition: Software-Defined Networking is an architectural approach that separates the network’s control plane (decision-making: "where should traffic go?") from the data plane (actual forwarding of packets).
...
•	Typical Use Cases: Data centers, cloud networks, WAN optimization, network automation, service chaining, fast reconfiguration, multi-tenancy.
Key Characteristics:
•	Centralized control and programmability; changes can be made network-wide with code or policies.
•	Supports virtual networks (which could use VLANs, VXLAN, MPLS, etc. underneath).
...
o	SDN can leverage VLANs as one way to segment traffic, but can also use other methods (like VXLAN, GRE, MPLS, etc.) and dynamically change them as needed.
